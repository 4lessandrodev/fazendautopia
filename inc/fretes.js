...
module.exports = {
  ...
  getFretes() {
    return new Promise((resolve, reject) => {
      conn.query(`
      SELECT * FROM tb_frete AS frete INNER JOIN tb_lojas As origin 
      ON frete.origin = origin.id INNER JOIN tb_regiao As regiao 
      ON frete.destin = regiao.id WHERE frete.enabled = 1
      `, (err, results) => {
        if (err) {
          reject(err);
        } else {
